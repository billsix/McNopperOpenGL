...
GLUSvoid GLUSAPIENTRY glusRandomSetSeed(const GLUSuint seed) { srand(seed); }
...
GLUSfloat GLUSAPIENTRY glusRandomNormalf(const GLUSfloat mean,
  ...
  x1 = glusRandomUniformf(GLUS_UNIFORM_RANDOM_BIAS,
                          1.0f - GLUS_UNIFORM_RANDOM_BIAS);
  x2 = glusRandomUniformf(0.0f, 1.0f);

  return mean + standardDeviation *
                    (sqrtf(-2.0f * logf(x1)) * cosf(2.0f * GLUS_PI * x2));
}
...
GLUSboolean GLUSAPIENTRY glusRandomHammersleyf(GLUSfloat result[2],
  ...
  // Check, if m is in the allowed range, as only 32bit unsigned integer is
  // supported.
  if (m == 0 || m > 32) {
    return GLUS_FALSE;
  }
  ...
  // Revert bits by swapping blockwise. Lower bits are moved up and higher bits
  // down.
  revertSample = (sample << 16u) | (sample >> 16u);
  revertSample = ((revertSample & 0x00ff00ffu) << 8u) |
                 ((revertSample & 0xff00ff00u) >> 8u);
  revertSample = ((revertSample & 0x0f0f0f0fu) << 4u) |
                 ((revertSample & 0xf0f0f0f0u) >> 4u);
  revertSample = ((revertSample & 0x33333333u) << 2u) |
                 ((revertSample & 0xccccccccu) >> 2u);
  revertSample = ((revertSample & 0x55555555u) << 1u) |
                 ((revertSample & 0xaaaaaaaau) >> 1u);
