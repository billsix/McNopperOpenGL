...
static GLUSvoid (*glusMouse)(GLUSboolean pressed, GLUSint button, GLUSint xPos,
                             GLUSint yPos) = NULL;
static GLUSvoid (*glusMouseWheel)(GLUSint buttons, GLUSint ticks, GLUSint xPos,
                                  GLUSint yPos) = NULL;
static GLUSvoid (*glusMouseMove)(GLUSint buttons, GLUSint xPos,
                                 GLUSint yPos) = NULL;

GLUSvoid GLUSAPIENTRY
glusWindowSetKeyFunc(GLUSvoid (*glusNewKey)(GLUSboolean pressed, GLUSint key)) {
  glusKey = glusNewKey;
}

GLUSvoid GLUSAPIENTRY glusWindowSetMouseFunc(GLUSvoid (*glusNewMouse)(
    GLUSboolean pressed, GLUSint button, GLUSint xPos, GLUSint yPos)) {
  glusMouse = glusNewMouse;
}

GLUSvoid GLUSAPIENTRY glusWindowSetMouseWheelFunc(GLUSvoid (*glusNewMouseWheel)(
    GLUSint buttons, GLUSint ticks, GLUSint xPos, GLUSint yPos)) {
  glusMouseWheel = glusNewMouseWheel;
}

GLUSvoid GLUSAPIENTRY glusWindowSetMouseMoveFunc(
    GLUSvoid (*glusNewMouseMove)(GLUSint buttons, GLUSint xPos, GLUSint yPos)) {
...
GLUSvoid GLUSAPIENTRY
glusWindowSetUpdateFunc(GLUSboolean (*glusNewUpdate)(GLUSfloat time)) {
...
GLUSvoid _glusWindowInternalReshape(GLFWwindow *window, GLUSint width,
                                    GLUSint height) {
  if (width < 1) {
    width = 1;
  }
...
GLUSvoid _glusWindowInternalKey(GLFWwindow *window, GLUSint key,
                                GLUSint scancode, GLUSint action,
                                GLUSint mods) {
...
GLUSvoid _glusWindowInternalMouse(GLFWwindow *window, GLUSint button,
                                  GLUSint action, GLUSint mods) {
...
GLUSvoid _glusWindowInternalMouseWheel(GLFWwindow *window, double xpos,
                                       double ypos) {
...
static GLUSvoid GLUSAPIENTRY glusWindowDebugMessage(
    GLUSenum source, GLUSenum type, GLUSuint id, GLUSenum severity,
    GLUSsizei length, const GLUSchar *message, const GLUSvoid *userParam) {
  glusLogPrint(GLUS_LOG_DEBUG,
               "source: 0x%04X type: 0x%04X id: %u severity: 0x%04X '%s'",
...
GLUSboolean GLUSAPIENTRY glusWindowCreate(
  ...
  while (walker && *walker != EGL_NONE) {
    ...
    case EGL_RENDERABLE_TYPE:
      if (*(walker + 1) == EGL_OPENGL_BIT) {
        ...
        glusLogPrint(GLUS_LOG_ERROR,
                     "EGL_RENDERABLE_TYPE has to be EGL_OPENGL_BIT");
    ...
    case EGL_SAMPLE_BUFFERS:
      if (*(walker + 1) >= 0 && *(walker + 1) <= 1) {
        ...
        glusLogPrint(GLUS_LOG_ERROR,
                     "EGL_SAMPLE_BUFFERS has to be >= 0 and <= 1");
    ...
    case EGL_SAMPLES:
      if (*(walker + 1) >= 0) {
        if ((eglSampleBuffersProcessed && samples == 1) ||
            !eglSampleBuffersProcessed) {
          samples = *(walker + 1);
        }
  ...
  if ((eglSampleBuffersProcessed && !eglSamplesProcessed) ||
      (!eglSampleBuffersProcessed && eglSamplesProcessed)) {
    glfwTerminate();

    glusLogPrint(GLUS_LOG_ERROR,
                 "EGL_SAMPLE_BUFFERS and EGL_SAMPLES has to be specified");
  ...
  while (walker && *walker != EGL_NONE) {
    ...
    case EGL_CONTEXT_MAJOR_VERSION:
      if (*(walker + 1) >= 1) {
        ...
        glusLogPrint(GLUS_LOG_ERROR,
                     "EGL_CONTEXT_MAJOR_VERSION has to be >= 1");
    ...
    case EGL_CONTEXT_MINOR_VERSION:
      if (*(walker + 1) >= 0) {
        ...
        glusLogPrint(GLUS_LOG_ERROR,
                     "EGL_CONTEXT_MINOR_VERSION has to be >= 0");
    ...
    case EGL_CONTEXT_OPENGL_FORWARD_COMPATIBLE:
      if (*(walker + 1) == EGL_TRUE || *(walker + 1) == EGL_FALSE) {
        ...
        glusLogPrint(GLUS_LOG_ERROR,
                     "EGL_CONTEXT_OPENGL_FORWARD_COMPATIBLE is invalid");
    ...
    case EGL_CONTEXT_OPENGL_PROFILE_MASK:
      if (*(walker + 1) == EGL_CONTEXT_OPENGL_CORE_PROFILE_BIT) {
        ...
        glusLogPrint(GLUS_LOG_ERROR,
                     "EGL_CONTEXT_OPENGL_PROFILE_MASK is invalid");
  ...
  if (!eglContextMajorVersionProcessed || !eglContextMinorVersionProcessed) {
    ...
    glusLogPrint(GLUS_LOG_ERROR,
                 "EGL_CONTEXT_CLIENT_VERSION, EGL_CONTEXT_MAJOR_VERSION or "
                 "EGL_CONTEXT_MINOR_VERSION not specified");
  ...
  g_window = glfwCreateWindow(width, height, title,
  ...
  if (!glfwWindowShouldClose(g_window)) {
    if (glusUpdate) {
      ...
      if (!glusUpdate(_glusWindowGetRecordingTime())) {
        ...
        if (_glusWindowGetCurrentRecordingFrame() <
            _glusWindowGetRecordingFrames()) {
          if (glusScreenshotUseTga(0, 0, _glusWindowGetRecordingImageTga())) {
            ...
            sprintf(filename, filenameTemplate,
...
GLUSint GLUSAPIENTRY glusWindowGetWidth(GLUSvoid) { return g_width; }

GLUSint GLUSAPIENTRY glusWindowGetHeight(GLUSvoid) { return g_height; }
