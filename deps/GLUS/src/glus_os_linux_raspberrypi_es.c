...
static void glusOsResizeDone(void *callback_data, uint32_t reason,
...
GLUSvoid _glusOsPollEvents() {
  ...
  while (SDL_PollEvent(&event)) {
    ...
    case SDL_KEYDOWN: {
      // CTRL-C
      if (event.key.keysym.sym == 99 && (event.key.keysym.mod == KMOD_LCTRL ||
      ...
      _glusWindowInternalKey(glusOsTranslateKey(event.key.keysym.sym),
                             GLFW_PRESS);
    } break;
...
EGLNativeWindowType _glusOsCreateNativeWindowType(
  ...
  if (fullscreen) {
    ...
    numberSupportedModes = vc_tv_hdmi_get_supported_modes_new(
        group, supportedModes, MAX_SUPPORTED_MODES, 0, 0);

    for (i = 0; i < numberSupportedModes; i++) {
      if (supportedModes[i].width == (uint32_t)width &&
          supportedModes[i].height == (uint32_t)height &&
          supportedModes[i].frame_rate >= 60) {
        break;
      }
    }
    ...
    if (vc_tv_hdmi_power_on_explicit_new(group, supportedModes[i].group,
                                         supportedModes[i].code) != 0) {
      vc_tv_unregister_callback(glusOsResizeDone);

      glusLogPrint(GLUS_LOG_ERROR, "Could not switch to full screen: %dx%d",
                   width, height);
  ...
  if (!SDL_SetVideoMode(videoInfo->current_w, videoInfo->current_h,
  ...
  dispmanElement = vc_dispmanx_element_add(
      dispmanUpdate, g_nativeDisplay, 0 /*layer*/, &dstRect, 0 /*src*/,
      &srcRect, DISPMANX_PROTECTION_NONE, &dispmanAlpha, 0 /*clamp*/,
      0 /*transform*/);
