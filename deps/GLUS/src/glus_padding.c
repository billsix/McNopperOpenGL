...
GLUSboolean GLUSAPIENTRY glusPaddingConvertf(GLUSfloat *target,
  ...
  for (currentElement = 0; currentElement < numberElements; currentElement++) {
    for (currentChannel = 0; currentChannel < channels + padding;
         currentChannel++) {
      if (currentChannel < channels) {
        target[currentElement * (channels + padding) + currentChannel] =
            source[currentElement * channels + currentChannel];
      } else {
