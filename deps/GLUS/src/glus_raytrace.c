...
GLUSboolean GLUSAPIENTRY glusRaytracePerspectivef(GLUSfloat *directionBuffer,
  ...
  for (i = 0; i < width * height; i++) {
    directionBuffer[i * (3 + padding) + 0] =
        -xExtend + xStep * 0.5f + xStep * (GLUSfloat)(i % width);
    directionBuffer[i * (3 + padding) + 1] =
        -yExtend + yStep * 0.5f + yStep * (GLUSfloat)(i / width);
...
GLUSvoid GLUSAPIENTRY glusRaytraceLookAtf(
  ...
  for (i = 0; i < width * height; i++) {
    ...
    if (directionBuffer && originDirectionBuffer) {
      glusMatrix3x3MultiplyVector3f(&directionBuffer[i * (3 + padding)],
                                    rotation,
                                    &originDirectionBuffer[i * (3 + padding)]);

      for (k = 0; k < padding; k++) {
        directionBuffer[i * (3 + padding) + 3 + k] =
            originDirectionBuffer[i * (3 + padding) + 3 + k];
      }
    }
  }
}
