...
extern EGLNativeWindowType _glusOsCreateNativeWindowType(
    const char *title, const GLUSint width, const GLUSint height,
...
GLUSvoid _glusWindowInternalClose(GLUSvoid) { g_done = GLUS_TRUE; }
...
GLUSboolean GLUSAPIENTRY glusWindowCreate(
  ...
  if (!glusEGLCreateContext(_glusOsGetNativeDisplayType(), &g_eglDisplay,
                            &eglConfig, &g_eglContext, configAttribList,
                            contextAttribList)) {
    glusWindowDestroy();
  ...
  eglGetConfigAttrib(g_eglDisplay, eglConfig, EGL_NATIVE_VISUAL_ID,
                     &nativeVisualID);

  eglNativeWindowType = _glusOsCreateNativeWindowType(
      title, width, height, fullscreen, noResize, nativeVisualID);
  ...
  if (!glusEGLCreateWindowSurfaceMakeCurrent(
          eglNativeWindowType, &g_eglDisplay, &eglConfig, &g_eglContext,
          &g_eglSurface, surfaceAttribList)) {
    glusWindowDestroy();
...
GLUSboolean GLUSAPIENTRY glusWindowLoop(GLUSvoid) {
  if (!g_done) // Loop That Runs While done=FALSE
  {
    ...
    eglSwapBuffers(g_eglDisplay,
                   g_eglSurface); // Swap Buffers (Double Buffering)
