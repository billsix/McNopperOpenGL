...
GLUSboolean glusFourierDFTc(GLUScomplex *result, const GLUScomplex *vector,
                            const GLUSint n) {
  if (!result || !vector) {
    return GLUS_FALSE;
  }
  ...
  if (n > 0) {
    ...
    GLUScomplex *dftMatrix =
        (GLUScomplex *)glusMemoryMalloc(n * n * sizeof(GLUScomplex));
    ...
    for (column = 0; column < n; column++) {
      for (row = 0; row < n; row++) {
        glusComplexRootOfUnityc(&dftMatrix[column * n + row], n, row * column,
                                -1.0f);
      }
    }
...
GLUSboolean glusFourierInverseDFTc(GLUScomplex *result,
  ...
  if (n > 0) {
    ...
    for (column = 0; column < n; column++) {
      for (row = 0; row < n; row++) {
        glusComplexRootOfUnityc(&dftInverseMatrix[column * n + row], n,
                                row * column, 1.0f);
      }
    }
...
static GLUSvoid glusFourierRecursiveFunctionFFTc(GLUScomplex *vector,
                                                 const GLUSint n,
                                                 GLUSint offset) {
  if (n > 1) {
    GLUSint i, k;
    ...
    for (i = 0; i < m; i++) {
      ...
      glusComplexMultiplyComplexc(&multiply, &currentW,
                                  &vector[offset + i + m]);
...
static GLUSvoid glusFourierButterflyShuffleFunctionFFTc(GLUScomplex *vector,
                                                        const GLUSint n,
                                                        const GLUSint offset) {
  if (n > 1) {
    GLUSint i, k;
...
static GLUSvoid glusFourierButterflyFunctionFFTc(GLUScomplex *vector,
                                                 const GLUSint n,
                                                 const GLUSint offset) {
  GLUSint currentStep;
  ...
  for (currentStep = 0; currentStep < steps; currentStep++) {
    for (currentSection = 0; currentSection < numberSections;
         currentSection++) {
      GLUScomplex currentW;
      currentW.real = 1.0f;
      currentW.imaginary = 0.0f;
      ...
      for (currentButterfly = 0; currentButterfly < numberButterfliesInSection;
           currentButterfly++) {
        GLUSint leftIndex =
            currentButterfly + currentSection * numberButterfliesInSection * 2;
        GLUSint rightIndex = currentButterfly + numberButterfliesInSection +
                             currentSection * numberButterfliesInSection * 2;
        ...
        glusComplexSubtractComplexc(&subtraction, &vector[leftIndex],
                                    &multiply);
...
static GLUSvoid glusFourierButterflyShuffleFunctionFFTi(GLUSint *vector,
                                                        const GLUSint n,
                                                        const GLUSint offset) {
  if (n > 1) {
    GLUSint i, k;
