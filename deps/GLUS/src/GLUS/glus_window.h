...
/**
 * Creates the window. In this function, mainly GLEW and GLFW functions are
 * used. By default, a RGBA color buffer is created.
 *
 * @param title				Title of the window.
 * @param width				Width of the window.
 * @param height			Height of the window.
 * @param fullscreen 		Flag for setting the window to fullscreen.
 * @param noResize 			GLUS_TRUE, if it should not be possible to
 * resize the window.
 ...
 */
GLUSAPI GLUSboolean GLUSAPIENTRY glusWindowCreate(
    const GLUSchar *title, const GLUSint width, const GLUSint height,
    const GLUSboolean fullscreen, const GLUSboolean noResize,
    const EGLint *configAttribList, const EGLint *contextAttribList,
    const EGLint *surfaceAttribList);

/**
 * Cleans up the window and frees all resources. Only needs to be called, if
 * creation of the window failed.
 */
GLUSAPI GLUSvoid GLUSAPIENTRY glusWindowDestroy(GLUSvoid);

/**
 * Starts the main loop. First the init, reshape, update and - if terminated -
 * terminate functions are called.
 *
...
/**
 * Starts everything before the main loop. The init and reshape functions are
 * called. Use this function, if glusRun can not be used.
 *
...
/**
 * Calls the main loop once. Update function is called and one screenshot is
 * taken and stored. Use this function, if glusRun can not be used.
 *
 * @return GLUS_ TRUE, if looping should continue.
 */
 ...
 * window. Has to be called after the window is created.
 *
 * @param interval Specifies the minimum number of video frames that are
...
/**
 * Sets the users initialization function. Called before the reshape and update
 * functions.
 *
 * The function must return GLUS_TRUE, if the initialization succeeded. If not,
 * the program terminates.
 */
GLUSAPI GLUSvoid GLUSAPIENTRY
    glusWindowSetInitFunc(GLUSboolean (*glusNewInit)(GLUSvoid));

/**
 * Sets the users reshape function. Called after the initialization and before
 * the update function. If the window is resized, this function is also called
 ...
 *
 * The time difference since last frame is passed in seconds and the resolution
 * is at least in milliseconds.
 *
 * If the function does not return GLUS_TRUE, the application is terminated.
 */
GLUSAPI GLUSvoid GLUSAPIENTRY
    glusWindowSetUpdateFunc(GLUSboolean (*glusNewUpdate)(const GLUSfloat time));

/**
 * Sets the users terminate function, which is called in any case. It can be
 * used to clean up resources.
 */
GLUSAPI GLUSvoid GLUSAPIENTRY
    glusWindowSetTerminateFunc(GLUSvoid (*glusNewTerminate)(GLUSvoid));
 ...
 */
GLUSAPI GLUSvoid GLUSAPIENTRY glusWindowSetKeyFunc(
    GLUSvoid (*glusNewKey)(const GLUSboolean pressed, const GLUSint key));

/**
 * Sets the users mouse handler.
 *
 * The function receives the state of a button and the x and y position in the
 * window.
 *
 * Buttons are:
 * 1 Left mouse button.
 ...
 */
GLUSAPI GLUSvoid GLUSAPIENTRY glusWindowSetMouseFunc(
    GLUSvoid (*glusNewMouse)(const GLUSboolean pressed, const GLUSint button,
                             const GLUSint xPos, const GLUSint yPos));

/**
 * Sets the users mouse wheel function.
 *
 * The function receives the states of all buttons, the direction of the wheel
 * in ticks and the current x and y position.
 */
GLUSAPI GLUSvoid GLUSAPIENTRY glusWindowSetMouseWheelFunc(
    GLUSvoid (*glusNewMouseWheel)(const GLUSint buttons, const GLUSint ticks,
                                  const GLUSint xPos, const GLUSint yPos));

/**
 * Sets the users mouse move function.
 *
 * The function receives the states of all buttons and the current x and y
 * position.
 */
GLUSAPI GLUSvoid GLUSAPIENTRY glusWindowSetMouseMoveFunc(
    GLUSvoid (*glusNewMouseMove)(const GLUSint buttons, const GLUSint xPos,
                                 const GLUSint yPos));
...
glusWindowStartRecording(GLUSint numberFrames, GLUSint framesPerSecond);
