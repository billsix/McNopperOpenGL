...
GLUSboolean GLUSAPIENTRY glusProgramCreateFromSource(
    ...
    const GLUSchar **controlSource, const GLUSchar **evaluationSource,
    const GLUSchar **geometrySource, const GLUSchar **fragmentSource) {
  GLUSint compiled;
  ...
  if (evaluationSource) {
    ...
    glShaderSource(shaderProgram->evaluation, 1,
                   (const char **)evaluationSource, 0);
    ...
    if (!compiled) {
      glGetShaderiv(shaderProgram->evaluation, GLUS_INFO_LOG_LENGTH,
                    &logLength);
      ...
      glGetShaderInfoLog(shaderProgram->evaluation, logLength, &charsWritten,
                         log);
  ...
  if (geometrySource) {
    ...
    glShaderSource(shaderProgram->geometry, 1, (const char **)geometrySource,
                   0);
    ...
    if (!compiled) {
      ...
      glGetShaderInfoLog(shaderProgram->geometry, logLength, &charsWritten,
                         log);
...
GLUSboolean GLUSAPIENTRY glusProgramBuildFromSource(
    ...
    const GLUSchar **controlSource, const GLUSchar **evaluationSource,
    const GLUSchar **geometrySource, const GLUSchar **fragmentSource) {
  if (!glusProgramCreateFromSource(shaderProgram, vertexSource, controlSource,
                                   evaluationSource, geometrySource,
                                   fragmentSource)) {
    return GLUS_FALSE;
  }
