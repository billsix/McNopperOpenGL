...
static GLUSvoid glusImageConvertRGB(GLUSubyte *rgbe, const GLUSfloat *rgb) {
  ...
  rgbe[0] = (GLUSubyte)(significant[0] * 256.0f *
                        powf(2.0f, (GLUSfloat)(exponent[0] - maxExponent)));
  rgbe[1] = (GLUSubyte)(significant[1] * 256.0f *
                        powf(2.0f, (GLUSfloat)(exponent[1] - maxExponent)));
  rgbe[2] = (GLUSubyte)(significant[2] * 256.0f *
                        powf(2.0f, (GLUSfloat)(exponent[2] - maxExponent)));
  rgbe[3] = (GLUSubyte)(maxExponent + 128);
}

static GLUSint glusImageDecodeNewRLE(FILE *file, GLUSubyte *scanline,
                                     GLUSint width) {
...
GLUSboolean GLUSAPIENTRY glusImageCreateHdr(GLUShdrimage *hdrimage,
  ...
  hdrimage->data = (GLUSfloat *)glusMemoryMalloc(width * height * depth *
                                                 stride * sizeof(GLUSfloat));
  if (!hdrimage->data) {
    return GLUS_FALSE;
  }
...
// see
// http://radiance-online.org/cgi-bin/viewcvs.cgi/ray/src/common/color.c?view=markup
...
GLUSboolean GLUSAPIENTRY glusImageLoadHdr(const GLUSchar *filename,
  ...
  hdrimage->data =
      (GLUSfloat *)glusMemoryMalloc(width * height * 3 * sizeof(GLUSfloat));
  ...
  while (y >= 0) {
    ...
    // Examine value
    if (width < 32768 && buffer[0] == 2 && buffer[1] == 2 &&
        buffer[2] == ((width >> 8) & 0xFF) && buffer[3] == (width & 0xFF)) {
      // New RLE decoding
...
GLUSboolean GLUSAPIENTRY glusImageSampleHdr2D(GLUSfloat rgb[3],
  ...
  _glusImageGatherSamplePoints(sampleIndex, sampelWeight, st, hdrimage->width,
                               hdrimage->height, stride);

  for (i = 0; i < stride; i++) {
    rgb[i] =
        hdrimage->data[sampleIndex[0] + i] * sampelWeight[0] * sampelWeight[1];
    rgb[i] += hdrimage->data[sampleIndex[1] + i] * (1.0f - sampelWeight[0]) *
              sampelWeight[1];
    rgb[i] += hdrimage->data[sampleIndex[2] + i] * sampelWeight[0] *
              (1.0f - sampelWeight[1]);
    rgb[i] += hdrimage->data[sampleIndex[3] + i] * (1.0f - sampelWeight[0]) *
              (1.0f - sampelWeight[1]);
  }
