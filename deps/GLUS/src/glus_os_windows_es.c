...
LRESULT CALLBACK _glusOsProcessWindow(HWND hWnd, UINT uiMsg, WPARAM wParam,
                                      LPARAM lParam) {
  switch (uiMsg) {
  case WM_CLOSE: {
    PostQuitMessage(0);
...
EGLNativeDisplayType _glusOsGetNativeDisplayType() { return g_nativeDisplay; }
...
EGLNativeWindowType _glusOsCreateNativeWindowType(
  ...
  g_nativeWindow =
      CreateWindowEx(dwExStyle, "GLES3", title, dwStyle, wRect.left, wRect.top,
                     fullWidth, fullHeight, NULL, NULL, hInstance, NULL);
