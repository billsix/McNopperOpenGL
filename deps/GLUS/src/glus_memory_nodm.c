...
#define GLUS_MEMORY_DATAT_ALIGNMENT_FACTOR                                     \
  (GLUS_MEMORY_DATA_ALIGNMENT_SIZE / GLUS_MEMORY_DATA_SIZE)
 ...
 */
static GLUSmemoryTable g_memoryTable[GLUS_MEMORY_TABLE_ENTRIES] = {
    {GLUS_VALID_AND_FREE, 0, GLUS_MEMORY_SIZE}};
...
static GLUSboolean glusMemoryInitTableEntry(size_t startIndex,
                                            size_t lengthIndices) {
  size_t tableIndex = 0;
      ...
      if (g_memoryTable[tableIndex].status == GLUS_VALID_AND_FREE) {
        ...
        while (otherTableIndex < g_memoryTableEntries) {
          ...
          if (g_memoryTable[otherTableIndex].status == GLUS_VALID_AND_FREE) {
            ...
            if (g_memoryTable[tableIndex].startIndex +
                    g_memoryTable[tableIndex].lengthIndices ==
                g_memoryTable[otherTableIndex].startIndex) {
              g_memoryTable[tableIndex].lengthIndices +=
                  g_memoryTable[otherTableIndex].lengthIndices;
...
static void *glusMemoryInternalMalloc(size_t size) {
  ...
  size_t lengthIndices = size % GLUS_MEMORY_DATA_SIZE == 0
                             ? (size / GLUS_MEMORY_DATA_SIZE)
                             : (size / GLUS_MEMORY_DATA_SIZE + 1);

  size_t alignmentIndicesRest =
      lengthIndices % GLUS_MEMORY_DATAT_ALIGNMENT_FACTOR;
  ...
  while (tableIndex < g_memoryTableEntries) {
    ...
    if (g_memoryTable[tableIndex].status == GLUS_VALID_AND_FREE &&
      ...
      if (lengthIndices < g_memoryTable[tableIndex].lengthIndices) {
        ...
        if (!glusMemoryInitTableEntry(
                g_memoryTable[tableIndex].startIndex + lengthIndices,
                g_memoryTable[tableIndex].lengthIndices - lengthIndices)) {
...
void GLUSAPIENTRY glusMemoryFree(void *pointer) {
  ...
  while (tableIndex < g_memoryTableEntries) {
    ...
    if (g_memoryTable[tableIndex].status == GLUS_VALID_AND_LOCKED &&
        (void *)&g_memory[g_memoryTable[tableIndex].startIndex] == pointer) {
      g_memoryTable[tableIndex].status = GLUS_VALID_AND_FREE;
