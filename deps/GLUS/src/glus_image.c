...
GLUSvoid _glusImageGatherSamplePoints(GLUSint sampleIndex[4],
                                      ...
                                      const GLUSfloat st[2], GLUSint width,
                                      GLUSint height, GLUSint stride) {
  ...
  if (pixelTexCoordCenter[0] > 0.5f &&
      pixelTexCoord[0] < pixelTexCoordCenter[0]) {
    sampleIndex[1] = sampleIndex[0] - stride;
    sampleIndex[3] = -stride;
  } else if (pixelTexCoordCenter[0] < (GLUSfloat)width - 0.5f &&
             pixelTexCoord[0] > pixelTexCoordCenter[0]) {
    sampleIndex[1] = sampleIndex[0] + stride;
    sampleIndex[3] = stride;
  } else {
  ...
  if (pixelTexCoordCenter[1] > 0.5f &&
      pixelTexCoord[1] < pixelTexCoordCenter[1]) {
    sampleIndex[2] = sampleIndex[0] - stride * width;
  } else if (pixelTexCoordCenter[1] < (GLUSfloat)height - 0.5f &&
             pixelTexCoord[1] > pixelTexCoordCenter[1]) {
    sampleIndex[2] = sampleIndex[0] + stride * width;
  } else {
