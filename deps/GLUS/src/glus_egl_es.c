...
EGLBoolean GLUSAPIENTRY glusEGLCreateContext(
  ...
  if (!eglDisplay || !eglConfig || !eglContext) {
    glusLogPrint(GLUS_LOG_ERROR,
                 "No eglDisplay, eglConfig or eglContext passed");
  ...
  context =
      eglCreateContext(display, config, EGL_NO_CONTEXT, contextAttribList);
...
EGLBoolean GLUSAPIENTRY glusEGLCreateWindowSurfaceMakeCurrent(
  ...
  if (!eglDisplay || !eglConfig || !eglContext || !eglSurface) {
    glusLogPrint(GLUS_LOG_ERROR,
                 "No eglDisplay, eglConfig, eglContext or eglSurface passed");
