...
static GLUSfloat glusPerlinGetInterpolatedValue(const GLUSfloat value0,
...
static GLUSfloat glusPerlinGetNoiseValue1D(GLUStgaimage *image, GLUSint x,
                                           const GLUSint amplitude,
                                           const GLUSint *data1D) {
  if (!image || !data1D) {
    return 0.0f;
  }
...
static GLUSfloat glusPerlinGetNoiseValue2D(GLUStgaimage *image, GLUSint x,
                                           GLUSint y, const GLUSint amplitude,
                                           const GLUSint *data2D) {
  if (!image || !data2D) {
    return 0.0f;
  }
...
static GLUSfloat glusPerlinGetNoiseValue3D(GLUStgaimage *image, GLUSint x,
                                           GLUSint y, GLUSint z,
                                           const GLUSint amplitude,
                                           const GLUSint *data3D) {
  if (!image || !data3D) {
    return 0.0f;
  }
  ...
  return (
      GLUSfloat)(data3D[x + y * (GLUSint)image->width +
                        z * (GLUSint)image->height * (GLUSint)image->width] %
                 amplitude);
}

GLUSboolean GLUSAPIENTRY glusPerlinCreateNoise1D(
  ...
  for (i = 0; i < octaves; i++) {
    ...
    for (x = 0; x < width; x += waveLenghtX) {
      ...
      p[0] = glusPerlinGetNoiseValue1D(image, xRandom,
                                       (GLUSint)currentAmplitude, data1D);
      p[1] = glusPerlinGetNoiseValue1D(image, xRandom + 1,
                                       (GLUSint)currentAmplitude, data1D);
      ...
      for (xInner = 0; xInner < waveLenghtX && x + xInner < width; xInner++) {
        ...
        data[index] += glusPerlinGetInterpolatedValue(
            p[0], p[1], (GLUSfloat)xInner / (GLUSfloat)waveLenghtX);
      }
    }
...
GLUSboolean GLUSAPIENTRY glusPerlinCreateNoise2D(
  ...
  for (i = 0; i < octaves; i++) {
    ...
    for (y = 0; y < height; y += waveLenghtY) {
      for (x = 0; x < width; x += waveLenghtX) {
        ...
        // Get random values from adjacent points
        p[0] = glusPerlinGetNoiseValue2D(image, xRandom, yRandom,
                                         (GLUSint)currentAmplitude, data2D);
        p[1] = glusPerlinGetNoiseValue2D(image, xRandom + 1, yRandom,
                                         (GLUSint)currentAmplitude, data2D);
        p[2] = glusPerlinGetNoiseValue2D(image, xRandom, yRandom + 1,
                                         (GLUSint)currentAmplitude, data2D);
        p[3] = glusPerlinGetNoiseValue2D(image, xRandom + 1, yRandom + 1,
                                         (GLUSint)currentAmplitude, data2D);

        // Sample the points in the wavelength area
        for (yInner = 0; yInner < waveLenghtY && y + yInner < height;
             yInner++) {
          for (xInner = 0; xInner < waveLenghtX && x + xInner < width;
               xInner++) {
            GLUSint index = (y + yInner) * width + x + xInner;

            GLUSfloat x0 = glusPerlinGetInterpolatedValue(
                p[0], p[1], (GLUSfloat)xInner / (GLUSfloat)waveLenghtX);
            GLUSfloat x1 = glusPerlinGetInterpolatedValue(
                p[2], p[3], (GLUSfloat)xInner / (GLUSfloat)waveLenghtX);

            data[index] += glusPerlinGetInterpolatedValue(
                x0, x1, (GLUSfloat)yInner / (GLUSfloat)waveLenghtY);
          }
        }
      }
    }
...
GLUSboolean GLUSAPIENTRY glusPerlinCreateNoise3D(
    GLUStgaimage *image, const GLUSint width, const GLUSint height,
    ...
    for (z = 0; z < depth; z += waveLenghtZ) {
      for (y = 0; y < height; y += waveLenghtY) {
        for (x = 0; x < width; x += waveLenghtX) {
          ...
          // Get random values from adjacent points
          p[0] = glusPerlinGetNoiseValue3D(image, xRandom, yRandom, zRandom,
                                           (GLUSint)currentAmplitude, data3D);
          p[1] = glusPerlinGetNoiseValue3D(image, xRandom + 1, yRandom, zRandom,
                                           (GLUSint)currentAmplitude, data3D);
          p[2] = glusPerlinGetNoiseValue3D(image, xRandom, yRandom + 1, zRandom,
                                           (GLUSint)currentAmplitude, data3D);
          p[3] = glusPerlinGetNoiseValue3D(image, xRandom + 1, yRandom + 1,
                                           zRandom, (GLUSint)currentAmplitude,
                                           data3D);

          p[4] = glusPerlinGetNoiseValue3D(image, xRandom, yRandom, zRandom + 1,
                                           (GLUSint)currentAmplitude, data3D);
          p[5] = glusPerlinGetNoiseValue3D(image, xRandom + 1, yRandom,
                                           zRandom + 1,
                                           (GLUSint)currentAmplitude, data3D);
          p[6] = glusPerlinGetNoiseValue3D(image, xRandom, yRandom + 1,
                                           zRandom + 1,
                                           (GLUSint)currentAmplitude, data3D);
          p[7] = glusPerlinGetNoiseValue3D(image, xRandom + 1, yRandom + 1,
                                           zRandom + 1,
                                           (GLUSint)currentAmplitude, data3D);

          // Sample the points in the wavelength area
          for (zInner = 0; zInner < waveLenghtZ && z + zInner < depth;
               zInner++) {
            for (yInner = 0; yInner < waveLenghtY && y + yInner < height;
                 yInner++) {
              for (xInner = 0; xInner < waveLenghtX && x + xInner < width;
                   xInner++) {
                GLUSint index = (z + zInner) * height * width +
                                (y + yInner) * width + x + xInner;

                GLUSfloat x0 = glusPerlinGetInterpolatedValue(
                    p[0], p[1], (GLUSfloat)xInner / (GLUSfloat)waveLenghtX);
                GLUSfloat x1 = glusPerlinGetInterpolatedValue(
                    p[2], p[3], (GLUSfloat)xInner / (GLUSfloat)waveLenghtX);
                GLUSfloat x2 = glusPerlinGetInterpolatedValue(
                    p[4], p[5], (GLUSfloat)xInner / (GLUSfloat)waveLenghtX);
                GLUSfloat x3 = glusPerlinGetInterpolatedValue(
                    p[6], p[7], (GLUSfloat)xInner / (GLUSfloat)waveLenghtX);

                GLUSfloat y0 = glusPerlinGetInterpolatedValue(
                    x0, x1, (GLUSfloat)yInner / (GLUSfloat)waveLenghtY);
                GLUSfloat y1 = glusPerlinGetInterpolatedValue(
                    x2, x3, (GLUSfloat)yInner / (GLUSfloat)waveLenghtY);

                data[index] += glusPerlinGetInterpolatedValue(
                    y0, y1, (GLUSfloat)zInner / (GLUSfloat)waveLenghtZ);
              }
            }
          }
        }
