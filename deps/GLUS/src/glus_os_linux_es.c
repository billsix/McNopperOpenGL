...
//
// Please note: The following lines are taken from GLFW. Some lines are modified
// for adapting to GLUS.
//
...
GLUSvoid _glusOsProcessWindow(const XEvent *msg) {
  ...
  case ConfigureNotify:
    ...
    if (msg->xconfigure.width != g_width ||
        msg->xconfigure.height != g_height) {
...
EGLNativeWindowType _glusOsCreateNativeWindowType(
  ...
  if (fullscreen) {
    ...
    screenConfiguration = XRRGetScreenInfo(
        g_nativeDisplay, RootWindow(g_nativeDisplay, defaultScreen));
    ...
    if (i == numberSupportedModes) {
      ...
      glusLogPrint(GLUS_LOG_ERROR,
                   "No matching display resolution found: %dx%d", width,
                   height);
    ...
    g_oldSizeID =
        XRRConfigCurrentConfiguration(screenConfiguration, &g_oldRotation);

    XRRSetScreenConfig(g_nativeDisplay, screenConfiguration,
                       RootWindow(g_nativeDisplay, defaultScreen), i,
  ...
  visualInfo = XGetVisualInfo(g_nativeDisplay, VisualIDMask,
                              &visualInfoTemplate, &numberOfVisuals);
  ...
  colormap = XCreateColormap(g_nativeDisplay,
                             RootWindow(g_nativeDisplay, defaultScreen),
  ...
  g_nativeWindow = XCreateWindow(
      g_nativeDisplay, RootWindow(g_nativeDisplay, defaultScreen), 0, 0, width,
      height, 0, visualInfo->depth, InputOutput, visualInfo->visual,
      CWBackPixel | CWBorderPixel | CWEventMask | CWColormap,
      &windowAttributes);

  XSetStandardProperties(g_nativeDisplay, g_nativeWindow, title, "", 0, 0, 0,
                         0);
  ...
  if (fullscreen) {
    ...
    Atom stateFullscreenMessage =
        XInternAtom(g_nativeDisplay, "_NET_WM_STATE_FULLSCREEN", False);
    ...
    XSendEvent(g_nativeDisplay, DefaultRootWindow(g_nativeDisplay), False,
...
GLUSvoid _glusOsDestroyNativeWindowDisplay() {
  if (g_nativeDisplay && g_fullscreen) {
    ...
    if (screenConfiguration) {
      XRRSetScreenConfig(g_nativeDisplay, screenConfiguration,
                         RootWindow(g_nativeDisplay, defaultScreen),
                         g_oldSizeID, g_oldRotation, CurrentTime);
...
double _glusOsGetRawTime() {
  ...
  return (double)currentTime.tv_sec +
