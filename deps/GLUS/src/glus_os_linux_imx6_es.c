...
GLUSvoid _glusOsPollEvents() {
  if (g_keyFileDescriptor >= 0) {
    ...
    do {
      numBytes =
          read(g_keyFileDescriptor, &keyEvent, sizeof(struct input_event));
      ...
      if (numBytes > 0 && keyEvent.type == EV_KEY) {
        ...
        case 1: // Pressed
        {
          ...
          if (keyEvent.code == 46 && (LEFT_CTRL || RIGHT_CTRL)) {
            ...
            _glusWindowInternalKey(glusOsTranslateKey(keyEvent.code),
                                   GLFW_PRESS);
          }
        } break;
  ...
  if (g_touchFileDescriptor >= 0) {
    ...
    do {
      numBytes =
          read(g_touchFileDescriptor, &touchEvent, sizeof(struct input_event));
      ...
      if (numBytes > 0) {
        if (touchEvent.type == 1) {
          ...
          if (touchEvent.code == 53) {
            ...
            submitPosition = 1;
          } else if ((touchEvent.code == 0 || touchEvent.code == 1) &&
                     submitPosition) {
            _glusWindowInternalMouseMove(g_currentX, g_currentY);
  ...
  if (g_powermateFileDescriptor >= 0) {
    ...
    do {
      numBytes = read(g_powermateFileDescriptor, &powermateEvent,
...
EGLNativeWindowType _glusOsCreateNativeWindowType(const char *title,
  ...
  glusLogPrint(GLUS_LOG_INFO,
               "Parameters 'title', 'fullscreen' and 'noResize' are not used");
  glusLogPrint(GLUS_LOG_INFO, "Key events are mapped to US keyboard");
  glusLogPrint(GLUS_LOG_INFO,
               "Touch display is used for mouse events and are limited");
