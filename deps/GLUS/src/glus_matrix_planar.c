...
GLUSvoid GLUSAPIENTRY glusMatrix4x4PlanarShadowPointLightf(
...
GLUSvoid GLUSAPIENTRY glusMatrix4x4PlanarShadowDirectionalLightf(
    GLUSfloat matrix[16], const GLUSfloat shadowPlane[4],
...
GLUSvoid GLUSAPIENTRY glusMatrix4x4PlanarReflectionf(
  ...
  rotationAngle =
      glusMathRadToDegf(acosf(glusVector3Dotf(reflectionPlane, yUpNormal)));
  ...
  if (pointOnPlane[0] != 0.0f || pointOnPlane[1] != 0.0f ||
      pointOnPlane[2] != 0.0f) {
    glusMatrix4x4Translatef(matrix, pointOnPlane[0], pointOnPlane[1],
                            pointOnPlane[2]);
  }

  if (rotationAngle != 0.0f) {
    glusMatrix4x4Rotatef(matrix, rotationAngle, rotationAxis[0],
