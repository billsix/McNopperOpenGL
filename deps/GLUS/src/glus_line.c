...
GLUSboolean GLUSAPIENTRY glusLineCreateLinef(GLUSline *line,
  ...
  line->vertices = (GLUSfloat *)glusMemoryMalloc(4 * line->numberVertices *
                                                 sizeof(GLUSfloat));
  line->indices =
      (GLUSindex *)glusMemoryMalloc(line->numberIndices * sizeof(GLUSindex));
...
glusLineCreateRectangularGridf(GLUSline *line, const GLUSfloat horizontalExtend,
  ...
  if (!line || rows < 1 || columns < 1 || numberVertices > GLUS_MAX_VERTICES ||
  ...
  for (i = 0; i <= rows; i++) {
    ...
    line->vertices[1 + i * 4 * 2 + 4] =
        -verticalExtend * 0.5f + rowStep * (float)i;
  ...
  for (i = 0; i <= columns; i++) {
    line->vertices[0 + i * 4 * 2 + offset * 4] =
        -horizontalExtend * 0.5f + columnStep * (float)i;
    ...
    line->vertices[0 + i * 4 * 2 + 4 + offset * 4] =
        -horizontalExtend * 0.5f + columnStep * (float)i;
...
GLUSboolean GLUSAPIENTRY glusLineCreateCirclef(GLUSline *line,
  ...
  if (!line || numberSectors < 3 || numberVertices > GLUS_MAX_VERTICES ||
      numberIndices > GLUS_MAX_INDICES) {
    return GLUS_FALSE;
  }
  glusLineInitf(line);
...
GLUSboolean GLUSAPIENTRY glusLineCopyf(GLUSline *line, const GLUSline *source) {
  ...
  if (source->numberVertices) {
    line->vertices = (GLUSfloat *)glusMemoryMalloc(4 * source->numberVertices *
                                                   sizeof(GLUSfloat));
    if (!line->vertices) {
      glusLineDestroyf(line);
    ...
    memcpy(line->vertices, source->vertices,
           4 * source->numberVertices * sizeof(GLUSfloat));
  }

  if (source->indices) {
    line->indices = (GLUSindex *)glusMemoryMalloc(source->numberIndices *
                                                  sizeof(GLUSindex));
    if (!line->indices) {
      glusLineDestroyf(line);
    ...
    memcpy(line->indices, source->indices,
