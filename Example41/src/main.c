...
// Note: If N and LENGTH is changed, the values has to be changed in the shaders
// as well.
...
GLUSboolean update(GLUSfloat time) {
  ...
  // Process all vertices. N groups as N rows are processed. One work group is
  // one row.
  ...
  // Process all vertices. N groups as N columns are processed. One work group
  // is one column.
  glDispatchCompute(1, N, 1);
