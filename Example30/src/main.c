...
// Every ray can have two sub rays (reflect and refract). This can be organized
// as a tree, with  breadth-first indexing. So, a tree with a depth has
// 2^depth-1 nodes. In this case we have a MAX_DEPTH of 5
#define NUM_STACK_NODES (2 * 2 * 2 * 2 * 2 - 1)

// As no recursion is possible in GLSL, the following is done:
// As the number of rays (= nodes) is known, all rays plus sub rays are
// executed. All needed values are stored in a stack node. After this is done,
// the tree is traversed again from the leaf node to the root. Now the color of
// node can be calculated by the using the sub nodes. Finally, in the root node
// the final color is stored.
...
Sphere g_sphereBuffer[NUM_SPHERES] = {
    // Ground sphere
    {.center = {0.0f, -10001.0f, -20.0f, 1.0f},
     .radius = 10000.0f,
...
GLUSboolean init(GLUSvoid) {
  ...
  // ... and bind this texture as an image, as we will write to it. see binding
  // = 0 in shader.
