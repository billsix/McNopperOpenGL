...
GLUSboolean init(GLUSvoid) {
  ...
  struct MaterialProperties material = {.ambientColor = {0.0f, 0.1f, 0.1f, 1.0f},
  ...
  glusFileLoadText(RESOURCE_PATH PATH_SEPERATOR "Example18" PATH_SEPERATOR "shader" PATH_SEPERATOR "phong.vert.glsl",
                   &vertexSource);
  glusFileLoadText(RESOURCE_PATH PATH_SEPERATOR "Example18" PATH_SEPERATOR "shader" PATH_SEPERATOR "phong.frag.glsl",
                   &fragmentSource);
...
GLUSboolean update(GLUSfloat time) {
  ...
  // Visible elements gets twice inverted, because of the front and back face.
  // So the value is 0. The clipped area only gets inverted by the back face.
  // The front face is clipped, so no second invert is done. The final value is
  // 255 (assume an 8bit stencil buffer).
  ...
  // Note: Bug in the AMD driver. The above disable is not working: "Values
  // written into gl_ClipDistance for planes that are not enabled have no
  // effect."
  //       For a workaround, I am setting a far plane, which does not clip
  //       anything.
...
int main(int argc, char *argv[]) {
  EGLint eglConfigAttributes[] = {
      ...
      EGL_STENCIL_SIZE, 8, EGL_SAMPLE_BUFFERS, 1, EGL_SAMPLES,   16, EGL_RENDERABLE_TYPE, EGL_OPENGL_BIT,
      EGL_NONE};
